    ...
    public function goOnline(Request $request)
    {
        ...
        try {
            $changed = false;
            $newOnlineStatus = DB::transaction(function () use ($user, &$changed) {
                if (!$user->is_online) {
                    $user->is_online = true;
                    $user->save();
                    $changed = true;
            ...
            $message = $changed ? 'You are now online and ready to serve customers.' : 'You were already online.';

            if ($request->expectsJson() || $request->is('*/counter/go-online')) {
                return response()->json([
                    'success' => true,
                    'message' => $message,
            ...
            Log::error('Error setting counter online: ' . $e->getMessage(), [
    ...
    public function goOffline(Request $request)
    {
        ...
        try {
            ...
            $message = $changed ? 'You are now offline.' : 'You were already offline.';

            if ($request->expectsJson() || $request->is('*/counter/go-offline')) {
                return response()->json([
                    'success' => true,
                    'message' => $message,
                    'is_online' => $newOnlineStatus
            ...
            Log::error('Error setting counter offline: ' . $e->getMessage(), [
                'user_id' => $user->id ?? null,
                'organization_id' => $organization->id ?? null
            ]);
            if ($request->expectsJson() || $request->is('*/counter/go-offline')) {
